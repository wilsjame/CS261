...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	if(cur == NULL){
		...
		else if(compare(val, cur->val) == 1){ //right > left
			cur->right = _addNode(cur->right, val);
		}
		else if(compare(val, cur->val) == 0){ 
			printf("The input value is already in the tree. Putting it to the right.\n");
			cur->right = _addNode(cur->right, val);
		}

		//at the end return cur, so that after the last subtree root is added
		//it recursively goes back up the tree, reaffirming each links pos
		return cur;
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	//compare root val with contains val
	//	return true in match
	//	if contains val is less than node val go left child
	//	if contains val is greater than node val go right child
	//	if node is null return false
	
	while(traverse != 0){

		switch(compare(traverse->val, val)){

			case -1: //left < right
				//contains value is grearer then node value go right
				traverse = traverse->right;
				break;
			case 1: //right > left
				//contains value is less than node value go left
...
{
	assert(cur != 0);
	struct Node *holdLeft = cur;
	assert(holdLeft != 0);

	//walk down left side of tree with root at cur and return the leftmost val
	while(holdLeft->left != 0){
 ...
 */
/*----------------------------------------------------------------------------*/
//returns tree with left most child removed
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	//		return left child
	//	else
	//		replace current value with value of the leftmost rightchild
	//		set right child to be removeLeftMost(current right)
	//else if value < current value
	//	leftchild = removeNode(leftChild, val)
	//else
	//	rightChild = removeNode(rightChild, value)
	//return current Node
	struct Node *temp;
