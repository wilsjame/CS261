...
/* returns the index of the smaller of two nodes
 * param: heap, pointer to heap
 * param: i, index of node
 * param: j, index of node
 * pre: i < size, j < size
 * return: index of the smaller node
*/
/*int indexSmallest(DynamicArray *heap, int i, int j, compareFunction compare)
{
...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    if(rightChild < last) //two children
    {
	    ...
	    {
		    //swap with smallest child, adjust heap
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
    // FIXED
    int parent;
    int pos = dySize(heap); // size index is the end of the array
